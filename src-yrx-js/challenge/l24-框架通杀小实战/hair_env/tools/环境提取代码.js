...
(function () {
  ...
  let get_obj_code = function (obj, obj_text, isdom) {
    ...
    function get_prototype_code(obj, obj_text) {
      ...
      js_code +=
        (!(obj_text in window) && !obj_text.includes(".") ? "let " : "") +
        `${obj_text}={};`;
      // 获取对象的所有描述符对象
      ...
      for (let _obj_attribute of Reflect.ownKeys(all_PropertyDescriptors)) {
        if (
          vm2_exies_var_arra.includes(_obj_attribute) &&
          obj_text === "window"
        ) {
        ...
        for (let _obj_attribute_descriptors in all_PropertyDescriptors[
          _obj_attribute
        ]) {
          // 获取描述内容
          let Descriptors_detail =
            all_PropertyDescriptors[_obj_attribute][_obj_attribute_descriptors];
          if (typeof Descriptors_detail === "function") {
            ...
            // 对value 为 函数的，进行额外处理，把value换成 get，以便于打印日志和补充环境
              ...
              Descriptors_text += `${keys[2]}: ${
                all_PropertyDescriptors[_obj_attribute][keys[2]]
              }, ${keys[3]}: ${
                all_PropertyDescriptors[_obj_attribute][keys[3]]
              }`;
              break;
            } else {
